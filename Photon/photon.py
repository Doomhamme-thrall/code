...
import requests
from core.colors import bad, end, good, green, info, red, run, white
...
from core.regex import rendpoint, rentropy, rhref, rintels, rscript
...
parser.add_argument(
    "-e", "--export", help="export format", dest="export", choices=["csv", "json"]
)
...
parser.add_argument(
    "-t", "--threads", help="number of threads", dest="threads", type=int
)
parser.add_argument(
    "-d", "--delay", help="delay between requests", dest="delay", type=float
)
parser.add_argument(
    "-v", "--verbose", help="verbose output", dest="verbose", action="store_true"
)
parser.add_argument(
    "-s", "--seeds", help="additional seed URLs", dest="seeds", nargs="+", default=[]
)
...
parser.add_argument(
    "--exclude", help="exclude URLs matching this regex", dest="exclude"
)
parser.add_argument(
    "--timeout", help="http request timeout", dest="timeout", type=float
)
parser.add_argument(
    "-p",
    "--proxy",
...
# Switches
parser.add_argument(
    "--clone", help="clone the website locally", dest="clone", action="store_true"
)
parser.add_argument(
    "--headers", help="add headers", dest="headers", action="store_true"
)
parser.add_argument(
    "--dns", help="enumerate subdomains and DNS data", dest="dns", action="store_true"
)
parser.add_argument("--keys", help="find secret keys", dest="api", action="store_true")
parser.add_argument(
    "--update", help="update photon", dest="update", action="store_true"
)
parser.add_argument(
    "--only-urls", help="only extract URLs", dest="only_urls", action="store_true"
)
parser.add_argument(
    "--wayback",
